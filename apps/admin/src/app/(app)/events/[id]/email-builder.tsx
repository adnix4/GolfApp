...
function buildEmailHtml(data: BuilderData, sections: SectionId[], subject: string): string {
  const primary   = data.primaryColor;
  const bodyParts = sections.map(id => {
    switch (id) {
      case 'header':
        return `
          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:${primary};border-radius:8px 8px 0 0;">
        ...
        return `
          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;">
            <tr><td style="padding:28px 32px;font-family:Arial,sans-serif;">
              <h2 style="color:${primary};font-size:20px;margin:0 0 12px;">${data.eventName}</h2>
      ...
      case 'mission':
        return data.missionStatement ? `
          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:#f9fafb;">
            <tr><td style="padding:20px 32px;font-family:Arial,sans-serif;border-left:4px solid ${primary};">
      ...
      case 'cta':
        return `
          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;">
            <tr><td style="padding:24px 32px;text-align:center;">
              <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" href="${data.registrationUrl}" style="height:44px;v-text-anchor:middle;width:200px;" arcsize="25%" strokecolor="${primary}" fillcolor="${primary}"><w:anchorlock/><center style="color:#ffffff;font-family:Arial,sans-serif;font-size:16px;font-weight:bold;">Register Now</center></v:roundrect><![endif]-->
              <!--[if !mso]><!--><a href="${data.registrationUrl}" style="background:${primary};color:#ffffff;display:inline-block;font-family:Arial,sans-serif;font-size:16px;font-weight:bold;padding:12px 32px;border-radius:8px;text-decoration:none;">Register Now</a><!--<![endif]-->
            </td></tr>
          </table>`;
        ...
        return `
          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:#f0f0f0;border-radius:0 0 8px 8px;">
            <tr><td style="padding:16px 32px;text-align:center;font-family:Arial,sans-serif;font-size:11px;color:#888;">
              ${data.orgName} · Powered by Golf Fundraiser Pro<br>
              <a href="${data.registrationUrl}" style="color:${primary};text-decoration:none;">${data.registrationUrl}</a>
            </td></tr>
          </table>`;
...
export default function EmailBuilderScreen() {
  ...
  useEffect(() => {
    fetch(`${BASE}/api/v1/events/${eventId}/email-builder/data`, {
      headers: { ...authHeader() },
    })
      .then(r => r.json())
      .then((d: BuilderData) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/api/v1/events/${eventId}/email-builder/send`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json', ...authHeader() },
        body:    JSON.stringify({ toAddress: toAddress.trim(), subject, html }),
      });
  ...
  const handleExport = async () => {
    ...
    const html = buildEmailHtml(builderData, sections, subject);
    const res = await fetch(`${BASE}/api/v1/events/${eventId}/email-builder/export`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json', ...authHeader() },
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.card, { backgroundColor: '#fff' }]}>
          ...
          <Text style={[styles.cardSub,   { color: theme.colors.accent  }]}>
            Drag-and-drop reordering coming soon. Use the arrows to reorder.
          </Text>
        ...
        <View style={[styles.card, { backgroundColor: '#fff' }]}>
          ...
          {sendStatus === 'error' && (
            <Text style={styles.errorMsg}>Failed to send. Check SENDGRID_API_KEY and try again.</Text>
          )}
        </View>

        {/* ── PREVIEW INFO ── */}
        <View style={[styles.card, { backgroundColor: '#f9fafb' }]}>
          <Text style={[styles.cardTitle, { color: theme.colors.primary }]}>Preview</Text>
          <Text style={[styles.cardSub,   { color: theme.colors.accent  }]}>
            Event: {builderData.eventName}{'\n'}
            Date: {builderData.eventDate}{'\n'}
            {builderData.eventLocation ? `Location: ${builderData.eventLocation}\n` : ''}
            Sponsors: {builderData.sponsors.length}{'\n'}
            QR URL: {builderData.registrationUrl}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  card:       { borderRadius: 12, padding: 20, gap: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
